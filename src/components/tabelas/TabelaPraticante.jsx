...
const TabelaPraticante = (props) => {
  ...
  const [listaAtualizada, setListaAtualizada] = useState(props.lista);

  useEffect(() => {
    const atualizarLista = async () => {
      const listaComStatus = await Promise.all(props.lista.map(async (item) => {
        const statusCadastro = await verificarSeEstaFinalizado(item.praticante.idPraticante);
        return {...item, statusCadastro};
      }));
      setListaAtualizada(listaComStatus);
    };
      ...
      <CTableBody>
        {
          listaAtualizada.map((item, key) => (
            <CTableRow key={key}>
              ...
              <CTableDataCell style={{verticalAlign: "middle", textAlign: "center"}}>
                <ModalComEvolucaoGraficoDeTorta nomeCompleto={item.nomeCompleto}
                                                idPraticante={item.praticante.idPraticante}/>
              </CTableDataCell>
              <CTableDataCell style={{verticalAlign: "middle", textAlign: "center"}}>
                <CButton color="" title={"Gerar relatório para " + item.nomeCompleto} style={{background: "none"}}
                         onClick={() => {
